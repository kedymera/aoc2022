...
int MaxPressure(struct Valve *valves, int numvalves, int **dists, int start1, int time1, int start2, int time2) {
    ...
    for (int i = 0; i < numvalves; ++i) {
        ...
        if (time1 >= time2) {
            int timeaftermove = time1 - dists[start1][i] - 1;
            maxpressure = max(maxpressure, timeaftermove * valves[i].rate + MaxPressure(valves, numvalves, dists, i, timeaftermove, start2, time2));
        }
        if (time2 > time1 || (time1 == time2 && start1 != start2)) {
            int timeaftermove = time2 - dists[start2][i] - 1;
            maxpressure = max(maxpressure, timeaftermove * valves[i].rate + MaxPressure(valves, numvalves, dists, start1, time1, i, timeaftermove));
        }
...
int main() {
    ...
    while (valves[start].id[0] != 'A' || valves[start].id[1] != 'A')
        ++start;
    printf("max pressure: %d\n", MaxPressure(valves, numvalves, dists, start, 26, start, 26));
