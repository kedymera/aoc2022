...
int MaximiseGeodes(int time,
    ...
    MAXGEODES = max(MAXGEODES, geode);
    ++calls;
    if (time <= 0) return geode;
    if (time <= 1) return geode+geoderobot;
    // OPTIMISATION: if current branch has no hope of beating the best, even if all remaining time was spent building
    //                  geoderobots, quit.
    if (geode + (geoderobot + geoderobot + time - 1) * time / 2 < MAXGEODES) return 0;

    if (time > 24) {
        for (int i = time; i < TOTTIME; ++i) printf(" ");
        printf("call%ld (MG%d): o%d+%d c%d+%d O%d+%d g%d+%d\n", calls, MAXGEODES, ore, orerobot, clay, clayrobot, obs, obsrobot, geode, geoderobot);
    }
